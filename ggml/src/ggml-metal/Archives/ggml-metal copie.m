        ...
        if (devices && [devices count] > 0) {
            ...
            if (chosen == nil) {
                ...
                if (@available(macOS 10.12, iOS 16.0, *)) {
                    // pick the device with the largest *available* working set (recommended - currently allocated)
// this tends to pick the “less busy” GPU on multi-GPU systems (e.g. W6800X Duo where one die may be driving displays)
uint64_t best_avail = 0;
uint64_t best_ws    = 0;
for (id<MTLDevice> d in devices) {
    const uint64_t ws   = d.recommendedMaxWorkingSetSize;
    const uint64_t cur  = d.currentAllocatedSize;
    const uint64_t avail = ws > cur ? (ws - cur) : 0;

    if (avail > best_avail || (avail == best_avail && ws > best_ws)) {
        best_avail = avail;
        best_ws    = ws;
        chosen     = d;
    }
