...
#if defined(MTLResourceHazardTrackingModeUntracked)
            opts |= MTLResourceHazardTrackingModeUntracked;
#endif
            id<MTLBuffer> buf = [heap->obj newBufferWithLength:size_aligned options:opts offset:heap->offs];
            if (buf == nil) {
...
static enum ggml_status ggml_metal_graph_compute(
    ...
    // On discrete PCIe GPUs, reducing this helps start parallel encoding earlier (useful with n_cb >= 4).
    int n_main = 128;
#if TARGET_OS_OSX
    bool __is_discrete = !ctx_dev->mtl_device.hasUnifiedMemory;
#if defined(MTLDeviceLocationSlot)
    if ([ctx_dev->mtl_device respondsToSelector:@selector(location)] && ctx_dev->mtl_device.location == MTLDeviceLocationSlot) { __is_discrete = true; }
#endif
    if ([ctx_dev->mtl_device respondsToSelector:@selector(isRemovable)] && ctx_dev->mtl_device.removable) { __is_discrete = true; }
    const char * __env_nmain = getenv("GGML_METAL_N_MAIN");
    if (__env_nmain) {
        const int v = atoi(__env_nmain);
        if (v > 0) n_main = v;
    } else if (__is_discrete && n_cb >= 4) {
        n_main = 32;
    }
#endif
...
static void ggml_backend_metal_buffer_set_tensor(ggml_backend_buffer_t buffer, struct ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    @autoreleasepool {
        ...
        const size_t stage_len = abs_offs + size;
        id<MTLBuffer> src = [ctx_dev->mtl_device newBufferWithBytesNoCopy:base length:stage_len options:MTLResourceStorageModeShared deallocator:nil];
        if (src == nil) {
        ...
        if (ctx_dev != NULL && ctx_dev->mtl_device != nil) {
            ...
            if (mtl_buf != nil && mtl_buf.storageMode == MTLStorageModePrivate) {
                ...
                if (abs_offs + size > (size_t) mtl_buf.length) {
                    ...
                    void * base = (void *) ((uint8_t *) tensor->data - offs);
                    const size_t stage_len = abs_offs + size;
                    id<MTLBuffer> dst = [ctx_dev->mtl_device newBufferWithBytesNoCopy:base length:stage_len options:MTLResourceStorageModeShared deallocator:nil];
                    if (dst != nil) {
                        id<MTLCommandQueue> q = ctx_dev->mtl_queue_copy;
                        if (q == nil) {
                            q = [ctx_dev->mtl_device newCommandQueue];
                    ...
                    if (ctx_dev != NULL && ctx_dev->mtl_device != nil) {
                        ...
                        if (mtl_buf != nil && mtl_buf.storageMode == MTLStorageModePrivate) {
                            ...
                            if (offs + size <= (size_t) mtl_buf.length) {
                                ...
                                const size_t stage_len = offs + size;
                                id<MTLBuffer> src_stage = [ctx_dev->mtl_device newBufferWithBytesNoCopy:base length:stage_len options:MTLResourceStorageModeShared deallocator:nil];
                                if (src_stage != nil) {
                                    id<MTLCommandQueue> q = ctx_dev->mtl_queue_copy;
                                    if (q == nil) {
                                        q = [ctx_dev->mtl_device newCommandQueue];
        ...
        if (size_aligned > 0) {
            {
                ...
                if (__src && (__is_discrete || __force_private)) {
                    MTLResourceOptions __priv_opts = MTLResourceStorageModePrivate;
#if defined(MTLResourceHazardTrackingModeUntracked)
                    __priv_opts |= MTLResourceHazardTrackingModeUntracked;
#endif
                    __dst = [device newBufferWithLength:size_aligned options:__priv_opts];
    ...
    if (size_aligned <= device.maxBufferLength) {
        ...
        if (__src && (__is_discrete || __force_private)) {
            MTLResourceOptions __priv_opts = MTLResourceStorageModePrivate;
                    ...
                    __priv_opts |= MTLResourceHazardTrackingModeUntracked;
#endif
                    __dst = [device newBufferWithLength:size_step_aligned options:__priv_opts];
            id<MTLCommandQueue> __q = ctx_dev->mtl_queue_copy;
                    if (__q == nil) { __q = [device newCommandQueue]; }
            id<MTLCommandBuffer> __cb = [__q respondsToSelector:@selector(commandBufferWithUnretainedReferences)] ? [__q commandBufferWithUnretainedReferences] : [__q commandBuffer];
            id<MTLBlitCommandEncoder> __blit = [__cb blitCommandEncoder];
